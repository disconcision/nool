import { Exp, erase, atom, comp } from "./Exp";
import {
  val,
  TransformResult,
  matches,
...
describe("test matches", () => {
  it("binary deconstruction", () => {
    ...
    expect(matches(pat_1, tree_1)).toEqual([
      ["a", branch_1],
      ["b", branch_2],
    ]);

    let pat_2 = p_comp([p_var("+"), p_var("a"), p_var("b")]);
    expect(matches(pat_2, tree_1)).toEqual([
      ["+", head],
      ["a", branch_1],
      ["b", branch_2],
    ]);
    ...
    let pat_3 = p_comp([
      p_const("+"),
      p_comp([p_const("+"), p_const("1"), p_var("b")]),
      p_var("c"),
    ]);
    expect(matches(pat_3, tree_3)).toEqual([
      ["b", branch_2],
      ["c", branch_3],
    ]);
  });
...
const expect_exp = (e_in: Exp, e_out: Exp) =>
  expect(erase(e_in)).toEqual(erase(e_out));

const v = (b: any, c: any) => val(0, 1, b, c);

describe("test hydrate", () => {
  it("basic hydration", () => {
    expect_exp(hydrate(p_var("a"), []), atom("a"));
    expect_exp(hydrate(p_const("a"), [v("a", atom("1"))]), atom("a"));
    expect_exp(hydrate(p_var("a"), [v("a", atom("1"))]), atom("1"));
    // duplicate bindings use first for now
    expect_exp(
      hydrate(p_var("a"), [v("a", atom("1")), v("a", atom("2"))]),
      atom("1")
    );
    expect_exp(
      hydrate(p_comp([p_const("+"), p_var("a"), p_var("b")]), [
        v("a", atom("1")),
        v("b", atom("2")),
      ]),
...
describe("test root transform", () => {
  let expect_exp = (e_in: TransformResult, e_out: TransformResult) =>
    expect(e_in == "NoMatch" ? "NoMatch" : erase(e_in)).toEqual(
      e_out == "NoMatch" ? "NoMatch" : erase(e_out)
    );
    ...
    expect_exp(
      transform(
        ...
        p_comp([
          p_const("+"),
          p_var("a"),
          p_comp([p_const("+"), p_var("b"), p_var("c")]),
        ]),
        p_comp([
          p_const("+"),
          p_comp([p_const("+"), p_var("a"), p_var("b")]),
